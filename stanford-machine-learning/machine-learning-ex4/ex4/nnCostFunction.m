...
Squ = [Theta1_r(:);Theta2_r(:)]'*[Theta1_r(:);Theta2_r(:)];

J = -((y_mat(:)'*log(h(:))+(1-y_mat(:))'*log(1-h(:))))/m+(lambda/(2*m))*Squ;


for i = 1:m
    delta_3(i,:) = h(i,:) - y_mat(i,:);
    Theta2_grad = Theta2_grad+delta_3(i,:)'*a_2(i,:);
    delta_2(i,:) = (delta_3(i,:)*Theta2_r).*sigmoidGradient(z_2(i, :));
